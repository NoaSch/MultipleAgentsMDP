 ...
 */
public class simpleDataMuleStateModelOLD implements FullStateModel {
      ...
      //  State st = state.copy();
       ...
       // DataMulesState currentState = (DataMulesState) origState.copy();
        ...
        for (Set<Integer> newBrokens : powerSet(canBeBroken))
        {
            ...
            //change the last repair for the new broken
            for(Integer in: newBrokens)
            {
                //currentState.timeFromLastRepair[in] = -1;
                if(!newBrokens.contains(in))
                    newBrokens.add(in);
            }
//if the action is repair
          ...
          for(int i = 0; i < actionsArr.length; i++)
          {
            ...
            StateTransitionProb stp = new StateTransitionProb(new GenericOOState(new DataMulesState(getLocsAfter(currentState,actionsArr), newLastRepair)), prob);

            result.add(new StateTransitionProb(new GenericOOState(new DataMulesState(getLocsAfter(currentState,actionsArr),newLastRepair)), prob));
    ...
    private Set<Integer> getCanBeBroken(DataMulesState currentState, Action action) {
        ...
        {
            if(/*currentState.timeFromLastRepair[i]!= -1 &&*/  currentState.timeFromLastRepair[i]>=GUARANTEED_REMAIN_OK) {
                result.add(i);
            }
        }
   ...
   /* private Set<Integer> findWorking(Set<Integer> brokenSensors) {
    ...
    private Integer[] addOne(Integer[] current, Set<Integer> except)
    {
        ...
        for (int i = 0; i < current.length; i++)
        {
            ...
            {
              if(current[i] != -1 && current[i] != 0 && current[i] < GUARANTEED_REMAIN_OK)
                  result[i] +=1;
            }
